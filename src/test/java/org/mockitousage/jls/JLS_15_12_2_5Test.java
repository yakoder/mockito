...
public class JLS_15_12_2_5Test {
    ...
    /**
     * The JLS §15.12.2.5 states that the compiler must chose the most specific overload in Java 6
     * or Java 7, but with generics in the matcher, <strong>javac</strong> selects the upper bound,
     * which is {@code Object}, as such javac selects the most generic method.
     *
     ...
     * <blockquote>
     *
     * <p>If more than one member method is both accessible and applicable to a method invocation,
     * it is necessary to choose one to provide the descriptor for the run-time method dispatch. The
     ...
     * <blockquote>
     *
     * <p>The definition of potential applicability goes beyond a basic arity check to also take
     ...
     * argument cannot be properly typed until after overload resolution. These rules allow the form
     * of the lambda expression to still be taken into account, discarding obviously incorrect
     ...
     *       e1, ..., ek by §18.5.4.
     *   <li>m2 is not generic, and m1 and m2 are applicable by strict or loose invocation, and
     *       where m1 has formal parameter types S1, ..., Sn and m2 has formal parameter types T1,
     *       ..., Tn, the type Si is more specific than Ti for argument ei for all i (1 ≤ i ≤ n, n =
     *       k).
     *   <li>m2 is not generic, and m1 and m2 are applicable by variable arity invocation, and where
     *       the first k variable arity parameter types of m1 are S1, ..., Sk and the first k
     ...
     * </ul>
     *
     * <p>The above conditions are the only circumstances under which one method may be more
