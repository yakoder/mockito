...
public class InlineBytecodeGenerator implements BytecodeGenerator, ClassFileTransformer {
     ...
     * This can cause a circularity: In order to check if an instance is a mock we need to look up
     * if this instance is registered in the {@code mocked} set. But to look up this instance, we
     * need to create key instances that rely on weak reference properties. Loading the later
     * classes will happen before the key instances are completed what will cause Mockito to check
     * if those key instances are themselves mocks what causes a loop which results in a circularity
     * error. This is not normally a problem as we explicitly check if the instance that we
     * investigate is one of our instance of which we hold a reference by reference equality what
     * does not cause any code execution. But it seems like the load order plays a role here with
     * unloaded types being loaded before we even get to check the mock instance property. To avoid
     * this, we are making sure that crucuial JVM types are loaded before we create the first inline
     * mock. Unfortunately, these types dependant on a JVM's implementation and we can only maintain
     * types that we know of from well-known JVM implementations such as HotSpot and extend this
     * list once we learn of further problematic types for future Java versions. To allow users to
     * whitelist their own types, we do not also offer a property that allows running problematic
     * tests before a new Mockito version can be released and that allows us to ask users to easily
     * validate that whitelisting actually solves a problem as circularities could also be caused by
     * other problems.
     */
