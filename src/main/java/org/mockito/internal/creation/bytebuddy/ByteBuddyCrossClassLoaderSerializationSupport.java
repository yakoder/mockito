...
class ByteBuddyCrossClassLoaderSerializationSupport implements Serializable {
     ...
     * </code></pre>
     *       <p>So, {@link ObjectOutputStream} will track the <code>writeReplace</code> method in
     *       the instance and execute it, which is wanted to replace the mock by another type that
     *       will encapsulate the actual mock. At this point, the code will return an {@link
     ...
     *       writeReplace</code> and execute it. <em>(For that matter disabling replacement via
     *       {@link ObjectOutputStream#enableReplaceObject(boolean)} doesn't disable the <code>
     *       writeReplace</code> call, but just just toggle replacement in the written stream,
     *       <strong><code>writeReplace</code> is always called by <code>ObjectOutputStream</code>
     *       </strong>.)</em>
     ...
     *       shouldn't replace itself again. <strong>This flag is local to this class</strong>,
     *       which means the flag of this class unfortunately needs to be protected against
     *       concurrent access, hence the reentrant lock.
     * </ol>
     *
     * @param mockitoMock The Mockito mock to be serialized.
     * @return A wrapper ({@link CrossClassLoaderSerializationProxy}) to be serialized by the
     *     calling ObjectOutputStream.
     ...
     * custom {@link MockitoMockObjectInputStream} that will be in charge of creating the mock
     * class.
     */
    ...
    /**
     * Special Mockito aware <code>ObjectInputStream</code> that will resolve the Mockito proxy
     * class.
     *
     * <p>
     *
     * <p>This specifically crafted ObjectInoutStream has the most important role to resolve the
     * Mockito generated class. It is doing so via the {@link #resolveClass(ObjectStreamClass)}
     * which looks in the stream for a Mockito marker. If this marker is found it will try to
     * resolve the mockito class otherwise it delegates class resolution to the default super
     * behavior. The mirror method used for serializing the mock is {@link
    ...
    public static class MockitoMockObjectInputStream extends ObjectInputStream {
         ...
         * proxy class as the <code>ObjectStreamClass</code> doesn't carry useful information for
         * this purpose.
         *
        ...
        /**
         * Hack the <code>name</code> field of the given <code>ObjectStreamClass</code> with the
         * <code>newProxyClass</code>.
         *
         * <p>The parent ObjectInputStream will check the name of the class in the stream matches
         * the name of the one that is created in this method.
         *
         * <p>The CGLIB classes uses a hash of the classloader and/or maybe some other data that
         * allow them to be relatively unique in a JVM.
         *
         * <p>When names differ, which happens when the mock is deserialized in another ClassLoader,
         * a <code>java.io.InvalidObjectException</code> is thrown, so this part of the code is
         * hacking through the given <code>ObjectStreamClass</code> to change the name with the
         * newly created class.
         *
    ...
    /**
     * Simple interface that hold a correct <code>writeReplace</code> signature that can be seen by
     * an <code>ObjectOutputStream</code>.
     *
